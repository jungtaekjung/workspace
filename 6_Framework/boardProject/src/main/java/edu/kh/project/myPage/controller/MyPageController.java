...
// 2. SessionStatus를 이용하여 {} 작성된 값 제거 시 필요


@Controller // 요청, 응답, 제어 클래스 + Bean 등록
@RequestMapping("/myPage") // /myPage로 시작하는 모든 요청 받음
public class MyPageController {

	@Autowired // MyPageService의 자식 MyPageSerivceImpl 의존성 주입(DI)
	...
	public String secession(String memberPw, @SessionAttribute("loginMember") Member loginMember,
		...
		if(result>0) {
			...
			Cookie cookie = new Cookie("saveId", "");// 만약 같은 이름의 쿠키가 존재한다면 덮어쓰기됨
			cookie.setMaxAge(0);  // 기존 쿠키 삭제
			cookie.setPath("/"); // 클라이언트가 어떤 요청을 할 때 쿠키가 첨부될지 경로(주소)를 지정
			resp.addCookie(cookie); // 응답 객체 이용해서 쿠키를 클라이언트에게 전달
									// -> 클라이언트 컴퓨터에 파일로 생성
			path +="/";
