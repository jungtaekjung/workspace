...
// - 해당 클래스를 Servlet 클래스로 등록하고(== <servlet>) 
// 어떤 요청 주소를 처리할지 지정함(== <servlet-mapping>)
@WebServlet("/login")
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// * 응답 화면을 만드는 Servlet의 일을 효율적으로 처리할 수 있는 JSP에게 넘겨줄 예정
		
		// RequestDispatcher : Servlet -> JSP로 HttpServletRequest 객체 
		// 					   / HttpServletResponse 객체를 발송(위임)하는 역할의 객체
		
		// req.getRequestDispatcher("JSP 경로");
		// - HttpServletRequest 객체가 생성될 때 내부에 자동으로 
		//   요청 발송자(RequestDispatcher)가 같이 생성됨
		
		// *** JSP 경로 작성 규칙 ***
		// - webapp 폴더를 기준으로 JSP파일까지의 모든 경로 작성
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/loginResult.jsp");
	
		// JSP에게 전송될 예정인 req객체에 result 변수 값을 담아서 같이 전달
