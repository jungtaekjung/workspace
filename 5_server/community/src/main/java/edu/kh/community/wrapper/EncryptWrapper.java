...
public class EncryptWrapper extends HttpServletRequestWrapper{
	...
	// 생성자가 작성되어 있으면 컴파일러가 기본적으로 기본생성자 추가 안함
	
	// -> Wrapper 클래스 생성 시 반드시 HttpServletRequest 객체를 매개변수로 전달해야 함
	...
	public String getParameter(String name) {
		...
		switch(name) {
		case "memberPw" : case "inputPw" : case "currentPw" : case "newPw" :
			value = getSha512(super.getParameter(name)); break;
		
		
		// 암호화 되는 경우가 아니라면 기존 getParameter() 메소드의 형태 유지
