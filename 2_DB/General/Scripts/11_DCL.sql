...
-- 사용자 명 : 각자이니셜_sample / 비밀번호 sample1234

CREATE USER jtj_sample IDENTIFIED BY sample1234;

-- 2. 생성된 계정에 접속하기 위한 접속 방법 추가
-- > 데이터 베이스 -> 새 데이터베이스 연결
-- ORA-01045: 사용자 JTJ_SAMPLE는 CREATE SESSION 권한을 가지고있지 않아서 로그인 실패

-- 3. [SYS 관리자 계정] 생성된 샘플 계정에 DB 접속 권한 부여 
GRANT CREATE SESSION TO jtj_sample;

-- 4. 다시 생성된 샘플 계정 접속 방법 추가
...
GRANT CREATE TABLE TO jtj_sample; 

ALTER USER jtj_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- ROLE : 권한의 묶음
--> 묶어둔 권한(ROLE)을 특정 계정에 부여 --> 해당 계정은 특정 권한을 가진 역할을 갖게됨
...
-- [SYS 관리자 계정]
GRANT CONNECT, RESOURCE TO jtj_sample;

----------------------------------------------------------------------------------------

-- 객체 권한
--> jtj_sample 계정에서 KH_JTJ 계정의 EMPLOYEE 테이블 조회

-- 1. [샘플 계정] KH_JTJ 계정의 EMPLOYEE 테이블 조회
SELECT * FROM KH_JTJ.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음


-- 2. [KH 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여
GRANT SELECT ON EMPLOYEE TO jtj_sample;

-- 3. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM KH_JTJ.EMPLOYEE; 

-- 4. [KH 계정] 부여했던 SELECT 권한 회수
REVOKE SELECT ON EMPLOYEE FROM jtj_sample;

-- 5. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM KH_JTJ.EMPLOYEE; --> 조회 안됨
