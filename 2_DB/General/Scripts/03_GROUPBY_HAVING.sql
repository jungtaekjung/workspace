...
-- 그룹 함수는 단 한개의 결과 값만을 산출하기 때문에 그룹이 여러 개일 경우 오류 발생
...
--> WHERE절은 각 컬럼 값에 대한 조건 (SELECT문 해석 순서 기억하기!!)
...
ORDER BY DEPT_CODE; -- 부서별 평균 급여가 300만 이상
...
-- 부서별 그룹의 급여 합계중 900만원을 초과하는 부서코드와 급여 합계 조회
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
