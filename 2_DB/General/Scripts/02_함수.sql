...
/*3*/SELECT EMP_NAME, EMAIL, LENGTH(EMAIL) "이메일 길이"
/*1*/FROM EMPLOYEE
...
-- 문자열 검색을 5번부터 시작해서 두번째에 나오는 'B'의 위치를 조회
...
-- 주어진 컬럼이나 문자열의 앞, 뒤, 양쪽에 지정된 문자를 제거
...
SELECT 242.424, ROUND(242.424) FROM DUAL; -- 소수점 첫째 자리에서 반올림
SELECT 242.424, ROUND(242.424,1) FROM DUAL; -- 소수점 첫째 자리까지 출력
SELECT 242.424, ROUND(242.424,2) FROM DUAL; -- 소수점 둘째 자리까지 출력

SELECT 242.424, ROUND(242.424,0) FROM DUAL; -- 소수점 첫째 자리에서 반올림
SELECT 242.424, ROUND(242.424,-1) FROM DUAL; -- 소수점 0번째 자리에서 반올림
SELECT 242.424, ROUND(242.424,-2) FROM DUAL; -- 소수점 -1번째 자리에서 반올림

-- CEIL(숫자 | 컬럼명) : 올림
-- FLOOR(숫자 | 컬럼명) : 내림
--> 둘다 소수점 첫째 자리에서 올림/내림 처리

SELECT 242.4, CEIL(242.4), FLOOR(242.4) FROM DUAL;

-- TRUNC(숫자 | 컬럼명 [,위치]) : 특정 위치 아래를 버림(절삭)

SELECT TRUNC(242.424,1), TRUNC(242.424, -1) FROM DUAL;

-- 버림, 내림 차이점
SELECT TRUNC(-242.4), FLOOR(-242.4) FROM DUAL;
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '2024/10/16') ) || '개월' 수강기간 FROM DUAL;
...
SELECT	EMP_NAME, 
			...
			CEIL(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)/12) ||'년차' "근무 년 차"
FROM EMPLOYEE;

-- ADD_MONTHS(날짜, 숫자) : 날짜에 숫자만큼의 개월 수를 더함 

SELECT ADD_MONTHS(SYSDATE, 42)+42 FROM DUAL;
...
SELECT TO_CHAR(2424, '99999' ) FROM DUAL;  -- 숫자 5칸, 오른쪽 정렬
SELECT TO_CHAR(2424, '00000' ) FROM DUAL;  -- 숫자 5칸, 오른쪽 정렬, 빈칸 0 추가

SELECT TO_CHAR(2424242, '9,999,999' ) FROM DUAL;  -- 자릿수 구분
SELECT TO_CHAR(2424242, 'L9,999,999' ) FROM DUAL;  -- 화폐기호
SELECT TO_CHAR(2424242, '$9,999,999' ) FROM DUAL;  -- 화폐기호
...
SELECT EMP_NAME, HIRE_DATE, TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일" (DY)') 
FROM EMPLOYEE; 
...
-- 2024/11/19 15시07분
SELECT TO_CHAR(TO_DATE('241119 150700','YYMMDD HH24MISS' ) ,'YYYY/MM/DD HH24"시" MI"분"' ) 
FROM DUAL;
...
SELECT TO_DATE('501119','RRMMDD') FROM DUAL;
...
-- EMPLYOEE 테이블에서
-- 기본 보너스를 받던 사원의 보너스를 0.7로
-- 보너스를 받지 못했던 사원의 보너스를 0.4로 변경하여
-- 이름, 기존 보너스, 변경된 보너스 조회

SELECT EMP_NAME, BONUS, NVL2(BONUS,0.7,0.4)
FROM EMPLOYEE;
...
-- 성별 구분 
SELECT EMP_NAME,
	CASE WHEN SUBSTR(EMP_NO,8,1)=1 THEN 'MAN'
			WHEN SUBSTR(EMP_NO,8,1)=2 THEN 'WOMAN'
	END 성별
	FROM EMPLOYEE;
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6'
ORDER BY JOB_CODE; -- 정렬 기준이 되는 컬럼은 반드시 SELECT절에 포함될 필요 X

-------------------------------------------------------------------------------------------------

/* 그룹 함수 */

-- 하나 이상의 행을 그룹으로 묶어서 연산하여 총합, 평균 등의 하나의 결과 행으로 반환하는 함수

-- SUM(숫자가 기록된 컬럼명) : 합계

-- EMPLOYEE 테이블에서 모든 직원의 급여 합 조회
...
--> 가장 낮은 급여, 가장 빠른 입사일, 알파벳 순서가 가장 빠른 이메일
...
SELECT	EMP_NAME, 
			JOB_CODE, 
			TO_CHAR(SALARY*12*(NVL(BONUS,0)+1),'L999,999,999')
			-- 급여 *(1+ 보너스) * 12	
FROM EMPLOYEE;
