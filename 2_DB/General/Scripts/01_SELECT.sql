...
-- DISTINCT : 조회시 컬럼에 포함된 중복 값을 한 번만 표시할 때 사용
...
-- 부서코드가 'D9'인 직원의
-- 사번, 이름, 부서코드, 전화번호 조회
SELECT EMP_ID, EMP_NAME,DEPT_CODE,PHONE FROM EMPLOYEE
WHERE DEPT_CODE='D9';
...
/* BETWEEN을 이용한 날짜 비교 */
-- EMPLOYEE 테이블에서
-- 입사일이 1990/01/1 ~ 1999/12/31( == 90년도 입사자)
-- 직원의 사번, 이름, 입사일 조회
 ...
 * '_'(글자 수)
 * - 'A_' : A 뒤에 아무거나 한 글자
 * '___A' : A 앞에 아무거나 세 글자(4글자 문자열 이면서 A로 끝나야 함)
 * 
 * 
 */
...
-- EMPLOYEE 테이블에서 메일 주소 '_'의 앞이 4글자 이면서
-- DEPT_CODE가 D9 또는 D6이고
-- 고용일이 90/01/01 ~00/12/01 이고
-- 급여 270만 이상인 사원의 이름, 이메일, 부서코드, 입사일, 급여를 조회
