...
-- EMPLOYEE 테이블 DEPT_CODE 컬럼과 DEPARTMENT 테이블에 DEPT_ID 컬럼은
-- 서로 같은 부서 코드를 나타낸다.
...
-- 조인되는 테이블의 각 행들이 모두 맵핑된 데이터가 검색되는 방법(곱집합)
...
-- 사번, 		이름, 			사수번호, 		사수 이름 조회
-- EMP_IN, EMP_NAME,   MANAGER_ID,   EMP_NAME

-- ANSI 표준
SELECT EMP.EMP_ID, EMP.EMP_NAME, NVL(EMP.MANAGER_ID,'없음') ,NVL(MG.EMP_NAME,'없음')
FROM EMPLOYEE EMP
LEFT JOIN EMPLOYEE MG ON(EMP.MANAGER_ID = MG.EMP_ID);
--> EMP 테이블에서 찾으려는 사수의 이름이 MG 테이블에 있다.
-- 오라클 구문
SELECT EMP.EMP_ID, EMP.EMP_NAME, NVL(EMP.MANAGER_ID,'없음') ,NVL(MG.EMP_NAME,'없음')
FROM EMPLOYEE EMP, EMPLOYEE MG

WHERE EMP.MANAGER_ID = MG.EMP_ID(+);
...
SELECT EMP_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE) -- "LOCATION_ID" : 부적합한 식별자
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE);
...
-- SELF JOIN 사용
SELECT E.EMP_NAME, E.DEPT_CODE, C.EMP_NAME 동료이름
FROM EMPLOYEE E
JOIN EMPLOYEE C ON (E.DEPT_CODE = C.DEPT_CODE)
