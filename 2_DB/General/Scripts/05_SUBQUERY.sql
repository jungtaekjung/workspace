...
-- 3) 위의 두 쿼리를 합쳐 부서별 급여 합이 큰 부서의 부서명, 급여 합 조회
...
-- 		서브쿼리 SELECT절에 나열된 컬럼의 수가 여러 개 일 때 							
...
SELECT EMP_NAME
FROM EMPLOYEE
WHERE (DEPT_CODE, JOB_CODE)= (	SELECT DEPT_CODE, JOB_CODE -- 다중 열 서브쿼리
												FROM EMPLOYEE
												WHERE ENT_YN='Y' 
												AND SUBSTR(EMP_NO,8,1)=2);	
...
-- 1) 급여를 200만원, 600만원 받는 직원 (200만, 600만원이 평균 급여라고 가정)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
-- 	5.  상[호연]관 서브쿼리 (메인쿼리 1행씩 우선해석, 서브쿼리 나중에 해석)
...
-- 부서별 입사일이 가장 빠른 사원의
-- 사번, 이름, 부서명(NULL 인경우 '소속없음'), 직급명, 입사일을 조회하고
-- 입사일이 빠른 순으로 조회
-- 단, 퇴사한 직원은 제외
...
WHERE (DEPT_CODE,HIRE_DATE)  IN (SELECT DEPT_CODE,MIN(HIRE_DATE)
												...
												WHERE ENT_YN = 'N'
												GROUP BY DEPT_CODE)
ORDER BY HIRE_DATE;						-- 부서별 입사일이 가장 빠른 날짜			
...
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
							...
							OR (SUB.DEPT_CODE IS NULL AND MAIN.DEPT_CODE IS NULL)) AND ENT_YN ='N')
...
-- 6. 스칼라 서브쿼리
-- SELECT절에 사용되는 서브쿼리 결과 1행(단일행)만 반환
...
--> SELECT 해석 순서를 고려하지 않아서 제대로 조회하지 않음
...
-- 해결 2) 해결 1의 조회 결과를 FROM절에 사용한 후 상위 5행만 조회
...
FROM EMPLOYEE
WHERE WHERE EXTRACT(YEAR FROM HIRE_DATE)=2000;
...
WHERE FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD' ) )/12) -- 나이
			= (SELECT MIN(FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD' ) )/12))	나이									
				FROM EMPLOYEE E2
				WHERE E1.JOB_CODE = E2.JOB_CODE )	-- 직급 별 가장 어린 직원
ORDER BY "만 나이" DESC;						
