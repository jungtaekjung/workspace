...
WHERE DEPT_TITLE = '경리부';

SELECT * FROM DEPARTMENT2;
SAVEPOINT SP1; -- 경리부 삭제 시점에 SAVEPOINT 지정

-- 전략기획팀 삭제
DELETE FROM DEPARTMENT2
WHERE DEPT_TITLE = '전락기획팀';
SELECT * FROM DEPARTMENT2;

-- 일반 ROLLBACK 시 경리부, 전략기획팀 복구
ROLLBACK;
SELECT * FROM DEPARTMENT2;
